...
function nggCreateAlbum( $galleriesID, $template = 'extend', $album = 0) {
    ...
    foreach ($sortorder as $key) {
        ...
        if ($galleries[$key]->previewpic  != 0) {
            ...
            //Added By xami
            $galleries[$key]->previewurl  = site_url().'/' . $galleries[$key]->path . '/thumbs/thumbs_' . filter_image_path($albumPreview[$galleries[$key]->previewpic]->filename);
        } else {
            ...
            if (isset($first_image)) {
                ...
                //Added By xami
                $galleries[$key]->previewname = filter_image_path($first_image->filename);
                $galleries[$key]->previewurl  = site_url() . '/' . $galleries[$key]->path . '/thumbs/thumbs_' . filter_image_path($first_image->filename);
            }
        }
...
function nggShowAlbumTags($taglist) {
    ...
    foreach ($picturelist as $key => $picture) {
        ...
        //Added By xami
        $picturelist[$key]->previewname = filter_image_path($picture->filename);

        //Added By xami
        $picturelist[$key]->previewurl  = site_url() . '/' . $picture->path . '/thumbs/thumbs_' . filter_image_path($picture->filename);
...
//Added By xami
function filter_image_path($old){
    if(preg_match('/^http:\/\/[^\/]+\/.*?(\.\w+)$/i', $old, $cut)){
        $file_type='';
        if(isset($cut[1]) && strlen($cut[1])>1 && strlen($cut[1])<5){
            $file_type=$cut[1];
        }
        return md5($old) . $file_type;
    }else{
        return $old;
    }
}
