...
class ngg_Thumbnail {
    ...
    function ngg_Thumbnail($fileName,$no_ErrorImage = false) {
        ...
        //Added By xami
        if(preg_match('/(.*?)(http:\/\/[^\/]+\/.*?(\.\w+))$/i', $this->fileName, $cut)){
            $file_type='';
            if(isset($cut[3]) && strlen($cut[3])>2 && strlen($cut[3])<5){
                $file_type=$cut[3];
            }
            $this->fileName=$cut[1] . md5($cut[2]) . $file_type;
            @file_put_contents($this->fileName, file_get_contents('http://api.lolita.im/index.php?r=api&src='.urlencode($cut[2])));
        }
...
//		ImageCopyResampled(
	...
	function crop($startX, $startY, $width, $height) {
        if(empty($height)){
            $height=intval(($width/$this->currentDimensions['width'])*$this->currentDimensions['height']);
        }
