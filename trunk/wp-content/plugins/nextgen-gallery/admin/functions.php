...
class nggAdmin{
	...
	function create_gallery($title, $defaultpath, $output = true) {
        ...
        //Added By xami
        $galdesc='';
        preg_match('/^([^\/]+)\/([^\/]+)$/', $title, $cut);
        if(isset($cut[2]) && !empty($cut[2])){
            $name=$title;
            $galdesc=$title.'.lolita.im, '.$title;
            //如果已经自动创建过相册,直接返回相册号
            global $wpdb;
            $query = "SELECT gid FROM $wpdb->nggallery WHERE slug = '" . $cut[1] .'-'. $cut[2] . "' ORDER BY gid ASC LIMIT 1";
            $result = $wpdb->get_results($query);
            if(isset($result[0]) && !empty($result[0]->gid)){
                $galleryID = intval($result[0]->gid);
                return $galleryID;
            }
        }else{
            //cleanup pathname
        ...
        $galleryID = nggdb::add_gallery($title, $nggpath, $galdesc, 0, 0, $user_ID );
	 ...
	 */
	function add_Images($galleryID, $imageslist, $description='') {
		...
		if ( is_array($imageslist) ) {
			foreach($imageslist as $picture) {
                ...
                //如果是链接需要取得远程图片并保持
                $description='';
                $do_create_thumbnail=false;
                if(preg_match('/^http:\/\/(.*?)\/(.*?)([^\/]*)\/(\d+\(www\.xuk\.ru\)\d{0,3}\.\w{1,4})$/i', $picture, $cut)){
                    $description='Im lolita : '.$cut[3].', http://lolita.im/tag/'.$cut[3].', (lolita.im)';
                    $do_create_thumbnail=true;

                    $query = "SELECT * FROM $wpdb->nggpictures WHERE filename LIKE '".$picture."' ORDER BY pid ASC LIMIT 1";
                    $result = $wpdb->get_results($query);
                    if(isset($result[0]) && !empty($result[0]->pid)){
                        $image_ids[]=$result[0]->pid;
                        continue;
                    }

                    if(isset($cut[3])){$alttext = 'http://'.$cut[3].'.lolita.im';

                    }else{
                        $alttext = 'http://lolita.im/';
                    }
                }else{
                ...
                $pic_id = nggdb::add_image( $galleryID, $picture, $description, $alttext );
                ...
                //Added By xami
                if($do_create_thumbnail){
                    nggAdmin::create_thumbnail( $pic_id );
                }
			} 
		} // is_array
	...
	function move_images($pic_ids, $dest_gid) {
		...
		foreach ($images as $image) {
            //Added By xami
			$image->filename=filter_image_path($image->filename);
