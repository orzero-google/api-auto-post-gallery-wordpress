...
class nggImage{
	...
	function nggImage($gallery) {			
        ...
        if(preg_match('/^http:\/\/[^\/]+\/.*?(\.\w+)$/i', $this->filename, $cut)){
            $this->filename=filter_image_path($this->filename);
            $this->imagePath	= WINABSPATH.$this->path . '/' . $this->filename;
            if(!file_exists($this->imagePath)){
                @file_put_contents($this->imagePath, file_get_contents('http://api.lolita.im/index.php?r=api&src='.urlencode($cut[0])));
            }
        }
